...
def plot_duration_by_phase(data_set: DataSet, title: str, file_size: int):
    ...
    for phase in RetrievalPhase:
        ...
        if len(sp_rets) > 0:
            sp_sample_sizes.append(len(sp_rets))
            sp_durations.append(calc.avg_duration(sp_rets, phase))
        else:
            sp_sample_sizes.append(0)
            sp_durations.append(0)

        if len(mp_rets) > 0:
            mp_sample_sizes.append(len(mp_rets))
            mp_durations.append(calc.avg_duration(mp_rets, phase))
...
def plot_duration_by_file_size(data_set: DataSet, title: str):
    ...
    for file_size in data_set.comparable_file_sizes:
        ...
        if len(sp_rets) > 0:
            sp_sample_sizes.append(len(sp_rets))
            sp_durations.append(calc.avg_duration(sp_rets, RetrievalPhase.TOTAL))
        else:
            sp_sample_sizes.append(0)
            sp_durations.append(0)
        ...
        if len(mp_rets) > 0:
            mp_sample_sizes.append(len(mp_rets))
            mp_durations.append(calc.avg_duration(mp_rets, RetrievalPhase.TOTAL))
